...
import { Game, Move, isGameDraw, isGameOver, getGameStatus, getWinningPattern } from "@/lib/contract";
...
export function PlayGame({ game }: PlayGameProps) {
  ...
  const [feedbackType, setFeedbackType] = useState<'win' | 'loss' | 'tie' | 'prediction'>('prediction');
  ...
  const winningPattern = gameStatus.isOver && gameStatus.winner ? getWinningPattern(board) || [] : [];
  ...
  return (
    <div className="flex flex-col gap-4 w-[400px]">
      <GameBoard
        ...
        disabled={gameIsOver || (!isMyTurn && !isJoinable) || isJoining || isPlaying}
      ...
      {showFeedback && (
        <div className={`
          ...
          ${feedbackType === 'win' ? 'bg-green-100 text-green-800 border-2 border-green-300' : ''}
          ${feedbackType === 'loss' ? 'bg-red-100 text-red-800 border-2 border-red-300' : ''}
          ${feedbackType === 'tie' ? 'bg-yellow-100 text-yellow-800 border-2 border-yellow-300' : ''}
          animate-pulse
        `}>
          {feedbackMessage}
        </div>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {gameIsDraw ? (
              ...
              >
                ...
                {game["winner"] === userData.profile.stxAddress.testnet && " (You won!)"}
              </Link>
            )}
          </div>
        )}
